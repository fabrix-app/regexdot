...
describe('#regexdot', () => {
  ...
  describe('## static', () => {
    it('should test static', () => {
      ...
      assert.equal(pattern.test('.books.'), true, '~> matches trailing dot');
      assert.equal(pattern.test('.books.author'), false, '~> does not match extra bits');
      assert.equal(pattern.test('books'), false, '~> does not match path without lead dot');
    })
  })
  describe('## multiples', () => {
    it('should test multiples', () => {
      ...
      assert.equal(pattern.test('.foo.bar.'), true, '~> matches trailing dot');
      assert.equal(pattern.test('.foo.bar.baz'), false, '~> does not match extra bits');
      assert.equal(pattern.test('foo.bar'), false, '~> does not match path without lead dot');
    })
    describe('## params', () => {
      describe('### param :: static', () => {
        describe('#### param :: static :: none', () => {
          it('should param :: static :: none', () => {
            ...
            assert.equal(pattern.test('.books.'), false, '~> does not match naked base w/ trailing dot')
            assert.equal(pattern.test('.books.narnia'), true, '~> matches definition')
            assert.equal(pattern.test('.books.narnia.'), true, '~> matches definition w/ trailing dot')
            assert.equal(pattern.test('.books.narnia.hello'), false, '~> does not match extra bits')
            assert.equal(pattern.test('books.narnia'), false, '~> does not match path without lead dot')
        ...
        describe('#### param :: static :: multiple', () => {
          it('should param :: static :: multiple', () => {
            ...
            assert.equal(pattern.test('.foo.bar.'), false, '~> does not match naked base w/ trailing dot')
            assert.equal(pattern.test('.foo.bar.narnia'), true, '~> matches definition')
            assert.equal(pattern.test('.foo.bar.narnia.'), true, '~> matches definition w. trailing dot')
            assert.equal(pattern.test('.foo.bar.narnia.hello'), false, '~> does not match extra bits')
            assert.equal(pattern.test('foo.bar.narnia'), false, '~> does not match path without lead dot')
      ...
      describe('### param :: multiple', () => {
        it('should param :: multiple', () => {
          ...
          assert.equal(pattern.test('.books.smith.'), false, '~> does not match insufficient paramters w. trailing dot');
          assert.equal(pattern.test('.books.smith.narnia'), true, '~> matches definition');
          assert.equal(pattern.test('.books.smith.narnia.'), true, '~> matches definition w. trailing dot');
          assert.equal(pattern.test('.books.smith.narnia.reviews'), false, '~> does not match extra bits');
          assert.equal(pattern.test('books.smith.narnia'), false, '~> does not match path without lead dot');
...
//
      describe('### param :: suffix', () => {
        it('should param :: suffix', () => {
          ...
          assert.equal(pattern.test('.movies.'), false, '~> does not match naked base w. trailing dot');
          ...
          assert.equal(pattern.test('.movies.foo.mp4.'), true, '~> does match with trailing dot');
        })
      ...
      describe('### param :: suffices', () => {
        it('should param :: suffices', () => {
          ...
          assert.ok(!pattern.test('.movies.'), '~> does not match naked base w. trailing dot');
          ...
          assert.ok(pattern.test('.movies.foo.mp4.'), '~> does match with trailing dot (mp4)');
          assert.ok(pattern.test('.movies.foo.mov'), '~> does match with correct suffix (mov)');
          assert.ok(pattern.test('.movies.foo.mov.'), '~> does match with trailing dot (mov)');
        });
      })
      describe('### param :: optional', () => {
        it('should param :: optional', () => {
          ...
          assert.ok(!pattern.test('.books.'), '~> does not match naked base w. trailing dot');
          assert.ok(pattern.test('.books.smith'), '~> matches when optional parameter is missing counts');
          assert.ok(pattern.test('.books.smith.'), '~> matches when optional paramter is missing w. trailing dot');
          assert.ok(pattern.test('.books.smith.narnia'), '~> matches when fully populated');
          assert.ok(pattern.test('.books.smith.narnia.'), '~> matches when fully populated w. trailing dot');
          assert.ok(!pattern.test('.books.smith.narnia.reviews'), '~> does not match extra bits');
          assert.ok(!pattern.test('books.smith.narnia'), '~> does not match path without lead dot');
        ...
        it('should param :: optional :: static :: none', () => {
          ...
          assert.deepEqual(keys, ['title'], '~> keys has "title" value');
          assert.ok(pattern.test('.'), '~> matches root w. trailing dot');
          assert.ok(pattern.test('.narnia'), '~> matches definition');
          assert.ok(pattern.test('.narnia.'), '~> matches definition w. trailing dot');
          assert.ok(!pattern.test('.narnia.reviews'), '~> does not match extra bits');
          assert.ok(!pattern.test('narnia'), '~> does not match path without lead dot');
        ...
        it('param :: optional :: multiple', () => {
          ...
          assert.ok(pattern.test('.books.horror.'), '~> matches when optional paramter is missing w. trailing dot');
          assert.ok(pattern.test('.books.horror.smith'), '~> matches when optional parameter is missing counts');
          assert.ok(pattern.test('.books.horror.smith.'), '~> matches when optional paramter is missing w. trailing dot');
          assert.ok(pattern.test('.books.horror.smith.narnia'), '~> matches when fully populated');
          assert.ok(pattern.test('.books.horror.smith.narnia.'), '~> matches when fully populated w. trailing dot');
          assert.ok(!pattern.test('.books.horror.smith.narnia.reviews'), '~> does not match extra bits');
          assert.ok(!pattern.test('books.horror.smith.narnia'), '~> does not match path without lead dot');
    ...
    describe('### wildcard', () => {
      it('wildcard', () => {
        ...
        assert.ok(pattern.test('.books.narnia.'), '~> matches definition w. trailing dot');
        assert.ok(pattern.test('.books.narnia.reviews'), '~> does not match extra bits');
        assert.ok(!pattern.test('books.narnia'), '~> does not match path without lead dot');
...
// ---
    ...
    it('(RegExp) static', () => {
      ...
      assert.ok(pattern.test('.books.'), '~> matches trailing dot');
      assert.ok(pattern.test('.books.'), '~> matches without leading dot');
...
//         assert.ok(pattern.test('/2019/'), '~> matches definition w/ trailing dot');
//         assert.ok(!pattern.test('2019'), '~> does not match without lead dot');
...
//       })
    ...
    it('(RegExp) nameless', () => {
      // For whatever reason~
      // ~> regexdot CANNOT give `keys` list because of unknown reason
      let rgx = /^\.books[\.]([^\.]\w+)[\.]?(\w+)?(?=\.|$)/i;
      let {keys, pattern} = regexdot(rgx);
      assert.deepEqual(keys, false, '~> keys = false');
      assert.deepEqual(rgx, pattern, '~> pattern = input');
